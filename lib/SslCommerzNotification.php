...
{
    protected $data = []; //this is also working as our $post_data array that contains all business and payment related information from our website
    protected $config = [];

    //variable to store re-directed urls on payment
    private $successUrl;
    private $cancelUrl;
    private $failedUrl;
    private $ipnUrl;
     ...
     */
    //constructor that initializes all the APIs (urls), storeID and storePassword provided by the parent company
    ...
    //validate your order. currency is BDT as this API is set for bangladesh
    //contains transaction_id(unique), amount, currency and post data array from checkout_hosted.php
    public function orderValidate($trx_id = '', $amount = 0, $currency = "BDT", $post_data)
    {
        ...
        //call validation method to check if the transaction is a valid one and not a fraud (send the same parameters)
    ...
    //the merchant referring here is us (from our website) - do remember that when a payment is made, we request SSLCommerze for a session for the customer
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            //all from post array data(checkout)
            $post_data['store_id'] = $this->getStoreId();
            $post_data['store_pass'] = $this->getStorePassword();

            //verify login to ur SSLCOMMERZ account
            if ($this->SSLCOMMERZ_hash_varify($this->getStorePassword(), $post_data)) {

                //if login successfull, set these values along with the api url (some of them will work when they receive a value)
                ...
                if ($code == 200 && !(curl_errno($handle))) {
                    ...
                    //on success page, these will be as e.g. $_POST['variable name']
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        if ($merchant_trans_currency == "BDT") { //check the currency
                            //then, check for security on the values that u need (these can be changed by adding more parameters)
                            ...
                            //This statement uses a different currency type. IF another currency is used instead of BDT, it will be converted based on the exchange rate
    ...
    //verify hash value for logging in
     ...
     */
    //customer will makePayment - this method handles the first stage of the payment
    //$requestData contains $post_data array. $type = hosted checkout payment integration
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        //set the Api urls for each stage of the transaction
        $this->setApiUrl($this->config['apiDomain'] . $this->config['apiUrl']['make_payment']);

        // Set the required/additional params - for customer, shipping, product and additional info
        $this->setParams($requestData); //pass in the array

        // Set the authentication information
        $this->setAuthenticationInfo(); //authenticate the payment (check whether the payment is valid or not)

        // Now, call the Gateway API (REST API)
    ...
    //Setting the urls for success, failed and cancelled transaction
    ...
    //set up the Instant Payment Notification (IPN)
    ...
    //SET ALL the information send from our website regarding the business and payment 
    ...
    //set the store_id and password of the account given to our company by the SSLCommerz (the company that provides the API)
    ...
    //-------all the functions below set payment info, customer info, product info, shipping info, addition info like VAT, etc---------
    
    public function setRequiredInfo(array $info)
    {
        $this->data['total_amount'] = $info['total_amount']; // decimal (10,2)	Mandatory - The amount which will process by SSLCommerz. It shall be decimal value (10,2). Example : 55.40. The transaction amount must be from 10.00 BDT to 500,000.00 BDT
        $this->data['currency'] = $info['currency']; // string (3 digits)	Mandatory - The currency type must be mentioned. It shall be three characters. Example : BDT, USD, EUR, SGD, INR, MYR, etc. If the transaction currency is not BDT, then it will be converted to BDT based on the current convert rate. Example : 1 USD = 82.22 BDT.
    ...
    public function setCustomerInfo(array $info)
    {
        ...
        $this->data['cus_zip'] = (isset($info['cus_zip'])) ? $info['cus_zip'] : null;
        $this->data['cus_items'] = (isset($info['cus_items'])) ? $info['cus_items'] : null;
        $this->data['cus_id'] = (isset($info['cus_id'])) ? $info['cus_id'] : null;
        $this->data['cus_order_pin'] = (isset($info['cus_order_pin'])) ? $info['cus_order_pin'] : null;
        $this->data['cus_order_identify'] = (isset($info['cus_order_identify'])) ? $info['cus_order_identify'] : null;
    ...
    public function setShipmentInfo(array $info)
    {
        ...
        $this->data['ship_date'] = (isset($info['ship_date'])) ? $info['ship_date'] : null;
    ...
    public function setProductInfo(array $info)
    {
        ...
        */
        ...
        $this->data['delivery_charge'] = (isset($info['delivery_charge'])) ? $info['delivery_charge'] : null;
