...
if (isset($_GET['edit'])) {
    $user_id = $_GET['edit'];
    $result = query("SELECT u1.name,u1.username,u1.user_email,u1.user_password,u1.user_image,d1.mobile_number,d1.address,d1.city,d1.salary
    FROM users u1
...
<?php
// update user information - driver edit only accessible to admin 
if (isset($_POST['update_submit'])) {
    ...
    $stmt2 = prepare_query("UPDATE delivery_man SET mobile_number=?,address=?,city=?,salary=? WHERE user_id=?");
    $stmt2->bindParam(1, $number, PDO::PARAM_STR);
    $stmt2->bindParam(2, $address, PDO::PARAM_STR);
    $stmt2->bindParam(3, $city, PDO::PARAM_STR);
    $stmt2->bindParam(4, $salary, PDO::PARAM_STR);
    $stmt2->bindParam(5, $user_id, PDO::PARAM_INT);
    $stmt2->execute();
    unset($stmt2);

    redirect("users_driver.php?success=user_update");
}
?>
        ...
        <div class="container-fluid">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="salary" value="<?php echo $old_salary; ?>">
                </div>
